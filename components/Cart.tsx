    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col">
            ...
            items.map(item => (
              <div key={item.id} className="flex gap-4 border-b border-gray-50 pb-4">
                ...
                <div className="flex-1">
                  <h4 className="text-[13px] font-bold uppercase leading-tight">{item.name}</h4>
                  <p className="text-[11px] text-gray-400">SKU: {item.sku}</p>
                  <div className="mt-2 flex justify-between items-center">
                    <div className="flex border border-gray-200 rounded-sm">
                      <button onClick={() => onUpdateQty(item.id, Math.max(1, item.quantity - 1))} className="px-3 py-1 hover:bg-gray-50">-</button>
                      <span className="w-8 text-center text-sm leading-8 font-bold">{item.quantity}</span>
                      <button onClick={() => onUpdateQty(item.id, item.quantity + 1)} className="px-3 py-1 hover:bg-gray-50">+</button>
                    </div>
                    <span className="text-sm font-bold">
                      {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(item.price * item.quantity)}
                    </span>
                  </div>
                  <button onClick={() => onRemove(item.id)} className="text-[10px] text-red-500 hover:underline mt-2 uppercase font-bold tracking-tighter">Remover Item</button>
                </div>
              </div>
            ))
          )}
        ...
        <div className="p-6 bg-gray-50 border-t space-y-4 shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.05)]">
          <div className="flex justify-between items-center text-[12px] uppercase tracking-wider text-gray-500 font-bold">
          ...
          {!isEligible && (
            <div className="bg-white border border-peach/30 p-4 rounded-sm">
               <div className="flex justify-between mb-1 text-[10px] font-bold uppercase tracking-widest text-peach">
          ...
          <button 
            ...
            className="w-full bg-black text-white py-5 font-black tracking-[0.2em] uppercase hover:bg-zinc-800 transition-all disabled:bg-gray-300 shadow-xl active:scale-[0.98] text-[12px]"
          >
