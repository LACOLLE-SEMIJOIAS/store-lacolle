...
const ProductCard: React.FC<ProductCardProps> = ({ product, onUpdate, isEditMode, onAddToCart }) => {
  ...
  // Placeholder elegante caso a imagem não carregue
    ...
    <div className={`group flex flex-col h-full bg-white transition-all duration-300 ${
      ...
      <div className="relative aspect-[3/4] overflow-hidden bg-gray-50">
        ...
        <div className="absolute top-4 left-4">
          <span className="bg-white/90 backdrop-blur-sm text-black text-[9px] px-3 py-1.5 tracking-[0.2em] font-bold shadow-sm">
            {product.sku}
          </span>
        </div>
        
        {/* SOBREPOSIÇÃO ESGOTADO */}
      ...
      <div className="py-4 flex flex-col gap-1">
        <h3 className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest truncate">
          {product.category}
        </h3>
        <h2 className="text-[11px] font-bold text-zinc-900 uppercase tracking-tight line-clamp-2 h-8">
          {product.name}
        </h2>
        ...
        <div className="mt-2 pt-2 border-t border-gray-50">
          {isEditMode ? (
            <div className="space-y-3">
               <div>
                  <label className="text-[8px] text-gray-400 font-bold uppercase">Preço Atacado</label>
                  <input 
                    type="number" 
                    value={product.price}
                    onChange={handlePriceChange}
                    className="w-full bg-gray-50 border border-gray-200 rounded px-2 py-1.5 text-xs font-bold focus:ring-1 focus:ring-peach outline-none"
                  />
               </div>
               <div>
                  <label className="text-[8px] text-gray-400 font-bold uppercase">Qtd Estoque</label>
                  <input 
                    type="number" 
                    value={product.stock}
                    onChange={handleStockChange}
                    className="w-full bg-zinc-900 text-white rounded px-2 py-1.5 text-xs font-bold focus:ring-1 focus:ring-peach outline-none"
                  />
               </div>
            </div>
          ) : (
            <div className="flex justify-between items-end">
              <div className="flex flex-col">
                <span className="text-[8px] text-gray-400 font-bold uppercase">Atacado</span>
